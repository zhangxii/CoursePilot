...
业务文件保存一个小组、成员、多道题目、课程资料、各题共享答案、评审和修改。Markdown 保存正文及 Front Matter，YAML 保存结构化状态，JSONL 保存会话；三者目录隔离，避免生命周期和查询职责混淆。
...
文件 repository 通过固定工作区文件确保不能创建第二个小组，通过题目索引维护唯一的 `active_assignment_id`。每道题的答案版本、评审和修改在独立目录内完成约束校验，再通过临时文件原子替换写入；同题只有一个版本链，不允许并行当前答案。
...
| 作业聚合 | 单小组 + 多题目 + 每题单一当前答案 | 题目可增加，但同一道题由小组共同维护一份答案 | 需要个人答案或答案分支时再引入分支模型 |
...
| Team, Assignments & Shared Answers | FR-007～FR-009、FR-030～FR-034 |
