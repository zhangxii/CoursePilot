...
└──────────────┬────────────────────┘
               ▼
┌──────────────── Infrastructure ─────────────────────────┐
│ Markdown Material Files │ SQLiteSession │ Business SQLite │ Trace │
└─────────────────────────────────────────────────────────┘
```
...
业务 SQLite 保存一个小组、成员、唯一大作业、课程资料路径、共享答案、评审和修改；资料正文始终是独立 Markdown 文件。建议会话表与业务表使用独立数据库文件或至少独立 repository，避免生命周期和查询职责混淆。
...
数据库通过固定单例键和唯一约束确保不能创建第二个小组或第二份作业。答案修改在一个事务中完成：插入 revision、插入新的共享 answer version、记录操作成员、更新上下文引用。资料保存采用本地状态 `pending → indexed`，失败进入 `failed` 并要求用户重新上传。
...
上传文件
 ...
 → 本地暂存
 → 附加 MaterialMetadata
 → 正文统一保存为 Markdown 文件，SQLite 只保存路径和元数据
