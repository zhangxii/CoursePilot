...
└──────────────┬────────────────────┘
               ▼
┌──────────────── Infrastructure ─────────────────────────┐
│ Local Material Library │ SQLiteSession │ Business SQLite │ Trace │
└─────────────────────────────────────────────────────────┘
```
...
1. 验证当前课程存在。
2. 使用当前课程过滤资料；总正文不超过字符预算时直接返回全文，否则最多返回 5 个相关页段。
3. 判断当前证据是否足以覆盖任务要求。
...
数据库通过固定单例键和唯一约束确保不能创建第二个小组或第二份作业。答案修改在一个事务中完成：插入 revision、插入新的共享 answer version、记录操作成员、更新上下文引用。资料解析采用本地状态 `pending → indexed`，失败进入 `failed` 并保留错误原因供重试。
...
上传文件
 ...
 → 生成带页码 Markdown
 → 附加 MaterialMetadata
 → 将 Markdown 正文和元数据保存到 SQLite
 → 小资料全文直传；超出字符预算时按页段进行关键词排序
```

预处理器实现统一接口，未来可增加 DOCX 或 OCR，不影响调用方。重复上传通过文件哈希和课程 ID 判断；同一文件内容更新时重新解析并保存新的本地正文。

## 9. 可靠性、安全与可观测性

- 对模型调用配置超时和有限重试；非幂等数据库写入不做盲目重试。
- 所有业务写入使用事务；资料解析和正文持久化均在本地完成。
