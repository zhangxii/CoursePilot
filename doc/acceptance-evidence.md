...
|---|---|---|---|
| 上传用户 `.md`/`.txt`/`.docx` 作业并保存原件/正文 | FR-044～FR-045 | 导入单测、文件 repository 集成测试、浏览器人工冒烟 | 待实现 |
| 用户稿创建正式版本且不覆盖历史 | FR-033、FR-045 | 版本事务测试、重启恢复测试 | 待实现 |
| Agent 结果先成为候选稿 | FR-025、FR-046 | Agent 编排测试、候选状态机测试 | 待实现 |
| 用户采用后才创建正式版本 | FR-047 | 采用事务成功/回滚/过期基础版本测试 | 待实现 |
| 正式版本和候选稿差异可查看 | FR-048 | 差异模块测试、Streamlit UI 测试 | 待实现 |
