...
| 多对话新建、切换、分支和归档 | FR-030～FR-031 | `tests/test_conversations.py` 覆盖空白/指定版本新建、切换、重命名、归档、精确消息分支、原子回滚与重启恢复；界面入口归入 M8 | 模块通过，UI 待 M8 |
| Agent 方案与用户方案对话隔离 | FR-049 | `tests/test_conversations.py` 已覆盖消息、题目、课程、基础版本和候选稿隔离；ProductionController 完整链路回归归入 T705 | 模块通过，产品链路待 T705 |
| 文字/文件优化方向与 Agent 问题分析 | FR-050～FR-052 | `test_optimization.py` 覆盖方向分库、来源、建议选择、双模式约束和基础目标；UI 待 M8 | 模块通过，UI 待 M8 |
| 生成/优化后独立自动审查和一次修正 | FR-023、FR-053 | `test_optimization.py` 覆盖独立输入契约、一次修正与复核；`test_app_flow.py` 验证生产生成后使用无 Session 审查 | 通过 |
| 自动审查与正式评审区分 | FR-054 | 自动/正式记录包含不同 `review_type` 与 `triggered_by`；展示待 M8 | 模块通过，UI 待 M8 |
