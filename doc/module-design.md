...
`PdfParser` 与 `PptxParser` 分别逐页返回 `PageContent(page_number, text)`；`MarkdownRenderer` 生成稳定页码标题；`MaterialIngestionService` 编排校验、哈希去重、解析、渲染、本地正文保存和状态更新。

异常类型至少包括：`UnsupportedFileType`、`FileTooLarge`、`EmptyExtraction`。失败记录应可重试，不删除诊断信息。
...
| `materials` | id, course_id, file_name, file_hash, type, status, index_status, content_markdown, error |
...
- 集成测试：SQLite repositories、PDF/PPTX 解析、本地全文/关键词检索、主 Agent 工具链（使用模型替身）。
