...
| `materials` | id, course_id, file_name, file_hash, type, status, index_status, storage_path, error |
...
配置项至少包含模型名、LLM Key、可选 OpenAI-compatible Base URL、数据库路径、全文字符预算、上传大小上限、检索结果上限、超时和重试次数。启动时验证必需配置，禁止在源代码中提供真实密钥默认值。

## 9. 测试设计

- 单元测试：意图到 Agent 路由、单例大作业约束、历史检索条件、过滤器构造、Pydantic 约束、页码渲染、共享答案版本事务。
- 集成测试：SQLite repositories、Markdown/纯文本文件存储、本地全文/关键词检索、主 Agent 工具链（使用模型替身）。
