...
| `ingestion` | 文件校验、逐页解析、Markdown 生成和本地保存 | FR-002～FR-005 |
| `services` | 小组、多题目、课程、每题共享答案、评审、修改用例 | FR-001、FR-006～FR-009、FR-032～FR-034 |
| `repositories` | Markdown/YAML 文件访问、约束和原子写入 | FR-031～FR-034 |
...
工具：`get_active_assignment`、`get_current_answer`、两个检索工具、`save_answer_version`。

处理顺序：读取当前题目和该题当前答案版本 → 提炼交付物和约束 → 检索当前课程标准 → 证据充分性判断 → 可选历史检索 → 完善共享答案 → 对照题目自检。输出 `AssignmentResult`，包括任务理解、共享答案、课程依据和不确定项。

### 3.4 ReviewAgent

工具：`get_active_assignment`、`get_current_answer`、`search_current_course`、`save_review`；只有符合历史检索条件时才可调用历史工具。
...
`MarkdownValidator` 接受 UTF-8 `.md` 和 `.txt`；`MaterialIngestionService` 编排校验、哈希去重、统一 `.md` 文件保存和状态更新，不负责解析 PDF/PPTX。

异常类型至少包括：`UnsupportedFileType`、`FileTooLarge`、`EmptyMarkdown`。失败记录保留诊断信息，用户可重新上传同一内容进行修复。
...
- `AnswerService`：按 assignment_id 保存初稿和共享答案版本，并保证每题只有一条当前版本链。
...
class AnswerRepository(Protocol):
    ...
    def get_latest(self, assignment_id: str) -> Answer | None: ...
...
1. 侧栏：当前课程、课程切换、配置状态。
2. 资料管理：元数据表单、上传进度、保存状态、失败后重新上传。
3. 对话区：消息流、当前意图、工具执行状态、最终结果。
4. 作业工作区：展示题目列表、新增/切换入口，以及当前题目的共享答案版本、评审结果、修改模式、操作成员和版本对比。
...
- 单元测试：意图到 Agent 路由、多题目隔离、每题单一当前答案、历史检索条件、过滤器构造、Pydantic 约束、页码渲染、共享答案版本事务。
- 集成测试：文件 repositories、Front Matter、JSONL Session、本地全文/关键词检索、主 Agent 工具链（使用模型替身）。
- 端到端测试：导入当前/历史资料后执行总结、完成、评审、修改，并检查 trace 和文件引用。
...
| FR-007～FR-009 | services, repositories, models, ui | 单小组、多题目和每题共享答案约束测试 |
