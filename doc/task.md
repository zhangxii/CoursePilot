...
本系统把正文保存为独立 Markdown 文件，SQLite 只保存路径与课程元数据；检索工具在本地强制执行 `course_id` 和 `status` 过滤。当前检索范围的总正文不超过配置预算时，全部交给模型；超过预算后才按关键词选取相关章节。
...
当前会话：SQLiteSession
业务数据：SQLite
长期资料：本地 Markdown 文件（SQLite 仅保存路径与元数据）
检索策略：当前课程优先，历史课程按需
