...
OpenAI Vector Store中的文件可以携带属性，并在语义检索时通过属性过滤；Agents SDK的`FileSearchTool`也支持过滤、排序选项和最大结果数。([OpenAI Developers][2])
...
Agents SDK的Session能够在多轮运行间维护会话历史，也有SQLite等持久化Session实现可用。([OpenAI GitHub Pages][3])
...
**Session是对话记忆，SQLite是业务记录，两者不要混为一谈。**

---

# 六、课程资料怎么导入

OpenAI File Search目前支持PDF、PPTX、DOCX、Markdown等文件，可直接建立向量检索。([OpenAI Developers][4])

但你的课程很依赖“第几页讲了什么”，所以建议不要只把原始PPT直接上传。
...
当前会话：SQLiteSession
业务数据：SQLite
长期资料：OpenAI Vector Store
检索策略：当前课程优先，历史课程按需
文件预处理：python-pptx + PyMuPDF
输出约束：Pydantic
调试：Agents SDK Tracing
```

Agents SDK的Tracing会记录模型生成、工具调用、Agent转交、护栏等事件，很适合你观察主Agent为什么选择某个资料库、为什么调用评审Agent，以及哪一步导致结果偏离。([OpenAI GitHub Pages][6])
