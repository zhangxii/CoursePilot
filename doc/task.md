...
Streamlit
OpenAI Agents SDK
SQLite 本地资料库（小规模全文输入，超预算后关键词检索）
SQLite
PyMuPDF + python-pptx
Pydantic
```
...
本系统把正文与课程元数据保存在 SQLite 中，检索工具在本地强制执行 `course_id` 和 `status` 过滤。当前检索范围的总正文不超过配置预算时，全部交给模型；超过预算后才按关键词选取相关章节。
...
系统首版支持 PDF、PPTX，并且课程很依赖“第几页讲了什么”，所以导入时先在本地预处理。
